...
Next, we will transform our data so that it is easier to work with when we begin analyzing it.
...
One of our variables is called `amenities` and contains a list of amenities for each entry: this is likely very valuable information, but we cannot work with the data as it is. Instead, we will make the most common amenities into dummy variables for each Airbnb.
...
That's really nice to look at! However, we still have some other things to take care of...
...
Then, we will create a column that turns `host_since` into lubridate format and another one with only the year:
...
Let's start by taking a look at the locations of our listings around the world:
...
Let's see how these cities vary among each other in their rating distributions:
...
Seems like people do not mind not having hot water! Or at least they do not expect to have it if it's not included within the amenities. Nevertheless, it's fun to look at!
...
In order to build our models, we will create a singular recipe that will tell each model how to use the data we are giving it what to predict. In our case, we will be predicting the `total_review_rating`. Here, I am choosing to exclude a few of the variables from our data set. I will be excluding:

-   `latitude` and `longitude` since they aren't continuous
...
bake(abnb_recipe, new_data = NULL) %>% head
```
...
In our last step before we finally build our models, we will be splitting our training data into folds. This step will provide us with mini training and testing sets that we can use to further train our models and get the best performance. This step increases our likelihood of getting better results in our actual testing set. I will only be creating three folds because of the size of our dataset to reduce the run time for our models:
...
Then, I'm going to save the training data, testing data, folds, and recipe in order to employ them later while building the actual models:

```{r, eval = FALSE}
save(abnb_tr, abnb_te, abnb_folds, abnb_recipe, file = "model_basics/model_setup.rda")
```
...
Our final model is our most complex and long winded: the random forest. We will follow the same steps to set it up starting with the engine and the model:
...
```{r, class.source = 'fold-hide'}
autoplot(rf_results) + scale_x_continuous(breaks = c(5,10))
```

Though they look pretty similar, we can see that models with more trees did better than the ones with less trees, and that more random predictors and greater minimal node sizes also gave us better values. Like before, let's look at the specific values of the best models:
...
The price of the Airbnb, whether the Host was a superhost, and the total amount of listings a Host has, seem to be the most decisive variables! It makes sense that the price of the Airbnb and a Host with a higher amount of experience would ensure a better stay for visitors.
...
We can see here that the Random Forest had the best results yet again! Let's look at the performance of the top rsq and rmse forests to choose the one we will use in the end:
...
rf2 <- rf_results %>% 
  ...
  subset(.metric == "rmse") %>% # select rmse
  arrange(mean) %>% # arrange by ascending
  slice(1:3) # select the lowest one

rf2 <- rbind(rf2, rf_results %>% # bind to rmse results
  collect_metrics() %>% # collect metrics
  subset(.metric == "rsq") %>% # select rsq
  arrange(desc(mean)) %>% # arrange by descending
  slice(1:3)) # select the highest one

rf2
```

After taking a quick peek, our forest with the highest rmse had a larger jump in value compared to other rmses. It also retained a almost the same exact rsq, making it our best contender! With 10 randomly selected predictors, a minimal node size of 17, and 500 trees, we will be fitting this random forest to our testing data!
...
WOW! Now that is a relief to look at. Our testing set took our rmse from 8.61 to 6.19 and our rsq from 0.16 to 0.70. The difference is really surprising, but it proves that our hard work paid off!
...
We can tell by this graph that our model does a poor job of predicting lower rated Airbnb review score ratings. Looking more closely, we can see that many of the values fall on multiples of 20, specifically 20, 40, and 60. This is likely because the specific Airbnbs only received one rating, which is why our model does a poor job of predicting them. In the future, if there is a possibility of adding review count to the dataset, it may increase our metrics even further.
...
As we can see, there had only been one review of the property which probably accounts for the poor overall rating! Though most of the information was the same, we can see that the Host changed the pricing of the Airbnb. Let's see what that does to our score:
...
It's current rating on the website is 4.60 stars or the equivalent of a 92 on our scale. This host had also changed the pricing of their Airbnb, as well as the minimum nights required, so we will make some changes to get a new prediction:
...
At the time, the Airbnb was rated a 99 and our model gave it a predicted score of 95.5. The rating of the Airbnb is now closer to 100 having 4.98 stars on the website. Let's see how different our predictions look after updating the price, total listings, and a few amenities:
...
We get 92.5 which is a difference of four points from the actual rating! This omission of information might account for lower prediction ratings in our model since their presence increases our score.
